<style type="text/css">
.list
{
	border:solid 1px #AAA; border-top:none;
	line-height:20px;
	padding:5px;
	background:#EFEFEF;
}
</style>
<BR>
<span style='font-weight:bold; line-height:30px; margin-top:40px;'>UPLOAD COMPLETE</span>
<BR>
<div style='text-align: left;'>
	<input type='button' value='กลับไปหน้าแรก' onclick='window.location="elder/hf_elderly";'>
	<input type='button' value='ย้อนกลับไปหน้านำเข้าข้อมูล' onclick='window.location="elder/hf_elderly/import";'>
</div>

<div style=' width:900px; margin-top:10px; border-top:solid 1px #AAA;'>
<? print_r($content); ?>
</div>
<div style='line-height:40px; font-weight:bold; cursor:pointer;' onclick='window.scrollTo(0,0);'>Go to top</div>


			

