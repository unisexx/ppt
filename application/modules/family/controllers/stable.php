<?php
Class stable extends Public_Controller
{
	...
	function index(){
		...
		$this->template->build('stable/index',$data);
	}
	...
	function form($id=FALSE){
		...
		$this->template->build('stable/form',$data);
	}
    
    function save(){
    	$_POST['key_id'] = $this->key_id;
    	$this->family->save($_POST);
		redirect('family/stable/index');
    }
	...
	function import_form(){
		...
		$this->template->build('stable/import_form',$data);
	}
	function import(){
//		$this->db->debug = true;
		//if(!menu::perm($this->menu_id, 'add') || !menu::perm($this->menu_id,'edit'))redirect('family/stable_index');
