...
<div id="search">
  <div id="searchBox">
   	<?php echo form_dropdown('year_data', get_year_option(null,null,'family WHERE key_id=30 ','year_data'), @$_GET['year_data'], null, '-- ทุกปี --'); ?>
    <?php echo form_dropdown('province_id', get_option('id', 'province', 'provinces', '1=1 order by province'), @$_GET['province_id'], null, '-- ทุกจังหวัด --'); ?>
  <input type="submit" name="button9" id="button9" title="ค้นหา" value=" " class="btn_search" /></div>
</div>
</form>
<div id="btnBox">
	<?php echo menu::perm($menu_id, 'import', 'family/medium/import_form'); ?>
    <?php echo menu::perm($menu_id, 'add', 'family/medium/form'); ?>
</div>
<?=$pagination;?>	
<table class="tblist">
<tr>
...
<tr  <? if($rowStyle =='')$rowStyle = 'class="odd"';else $rowStyle = "";echo $rowStyle;?>>
  ...
  <td><?=anchor('family/medium/form/'.$item['id'], $item['province_name']);?></td>
  ...
  <td>
  	...
  	<input type="submit" name="button9" id="button9" title="แก้ไขรายการนี้" value=" " class="btn_edit vtip"  onclick="window.location='family/medium/form/<?=$item['id'];?>'" />
  	<? endif;?>
  	<?php echo menu::perm($menu_id, 'delete', 'family/medium/delete/'.$item['id']); ?>
    </td>
</tr>
<? $i++;} ?>
</table>
